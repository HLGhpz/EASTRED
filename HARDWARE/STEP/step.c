...
void speed_up(int ia,s32 step)//四个电机一起加速
{
    ...
    {
        case 0:     SetpMotor_SetStep(0,step);	//后退
                    SetpMotor_SetStep(1,step);
                    SetpMotor_SetStep(2,-step);
                    SetpMotor_SetStep(3,-step);
                    break;
        case 1:     SetpMotor_SetStep(0,-step);	//前进
                    SetpMotor_SetStep(1,-step);
                    SetpMotor_SetStep(2,step);
                    SetpMotor_SetStep(3,step);
                    break;
        case 2:     SetpMotor_SetStep(0,step);	//左移
                    SetpMotor_SetStep(1,-step);
                    SetpMotor_SetStep(2,step);
                    SetpMotor_SetStep(3,-step);
                    break;
        case 3:     SetpMotor_SetStep(0,-step);	//右移
                    SetpMotor_SetStep(1,step);
                    SetpMotor_SetStep(2,-step);
                    SetpMotor_SetStep(3,step);
                    break;
                    ...
                    SetpMotor_SetStep(2,-step);
                    SetpMotor_SetStep(3,-step);
    for(Count = 0; Count<510; Count++)//每隔2ms重复设定速度值，越小越快
    {
        Speed=value[Count];
        Speed=value[Count];
        Speed=value[Count];
